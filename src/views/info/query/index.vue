      ...
      <div class="report-preview" v-loading="detailLoading">
        <div v-if="currentReport" class="report-header">
          <el-descriptions :column="2" border class="report-meta">
            <el-descriptions-item label="报告编号">{{ currentReport.reportId }}</el-descriptions-item>
            <el-descriptions-item label="客户名称">{{ currentReport.customerName }}</el-descriptions-item>
            <el-descriptions-item label="产品类型">{{ currentReport.productType }}</el-descriptions-item>
...
async function getList() {
  ...
  try {
    ...
    ElMessage.error('获取报告列表失败：' + (e.message || ''))
  } finally {
    loading.value = false
  }
}
